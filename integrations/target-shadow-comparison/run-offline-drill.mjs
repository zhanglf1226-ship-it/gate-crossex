...
try {
  for (const [index, scenario] of fixture.scenarios.entries()) {
    ...
    const shadowAggregate = aggregate(plan.actions), legacyAggregate = aggregate(legacy.actions);
    const expectedKinds = scenario.expectedKinds.join(','), actualKinds = plan.actions.map((action) => action.kind).join(','), exposureSemanticStatus=JSON.stringify(shadowAggregate)===JSON.stringify(legacyAggregate)?'MATCH':'DIFFERENT', orderingStatus=actualKinds===expectedKinds&&JSON.stringify(compactKinds(plan.actions))===JSON.stringify(compactKinds(legacy.actions))?'MATCH':'DIFFERENT', shadowPhaseStatus=validShadowPhases(plan.actions)?'MATCH':'DIFFERENT', executionShapeStatus=JSON.stringify(shape(plan.actions))===JSON.stringify(shape(legacy.actions))?'MATCH':'DIFFERENT';
    results.push({ id: scenario.id, status:[exposureSemanticStatus,orderingStatus,shadowPhaseStatus,executionShapeStatus].every((value)=>value==='MATCH')?'MATCH':'DIFFERENT', exposureSemanticStatus,orderingStatus,shadowPhaseStatus,executionShapeStatus,legacyPhaseEvidence:'UNAVAILABLE_ROUTER_SEQUENCE_ONLY',expectedKinds,shadowKinds:actualKinds,shadowActions:plan.actions,legacyActions:legacy.actions,shadowAggregate,legacyAggregate,shadowShape:shape(plan.actions),legacyShape:shape(legacy.actions),clipCountDifference:plan.actions.length-legacy.actions.length });
    database.close();
  }
} finally {

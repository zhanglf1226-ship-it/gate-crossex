...
function request(overrides: Partial<{
  method: string; url: string; body: unknown; userId: string; role: 'viewer' | 'planner' | 'approver' | 'admin' | 'auditor'; timestamp: string; nonce: string;
}> = {}) {
...
describe('CloudRequestAuthenticator', () => {
  it('authenticates a signed request and enforces roles', () => {
    ...
    expect(principal).toEqual({ userId: 'felix', role: 'planner' });
  ...
  it('rejects expired and replayed requests', () => {
    const auth = new CloudRequestAuthenticator(secret);
    const signed = request();
    expect(auth.authenticate(signed, now)).toEqual({ userId: 'felix', role: 'planner' });

...
describe('database migrations', () => {
  it('applies migrations once and reports the current version', () => {
    ...
    expect(readDatabaseStatus(first)).toEqual({
      state: 'ok',
      migrationCount: 24,
      currentMigration: '0024_imported_account_snapshots.sql',
    });
  ...
  it('repairs the legacy audit table created by the original bootstrap code', () => {
    ...
    expect(readDatabaseStatus(database).currentMigration).toBe('0024_imported_account_snapshots.sql');
    database.close();
  });
  ...
  it('prunes expired terminal execution data without touching active strategies', () => {
    ...
    expect(result).toEqual({
      ...
      protectionReconciliationsDeleted: 0,
      importedAccountSnapshotsDeleted: 0,
    });
